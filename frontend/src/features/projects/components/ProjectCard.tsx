...
export function ProjectCard({ project, onDelete }: ProjectCardProps) {
  ...
  return (
    <motion.div
      ...
      className="group relative glass-card rounded-2xl border border-white/[0.07] overflow-hidden flex flex-col justify-between"
    >
      ...
      <div className="p-5 space-y-4">
        <div className="flex items-start justify-between gap-3">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <Link href={`/projects/${project.id}`} className="font-semibold text-base text-white hover:text-cyan-400 transition flex items-center gap-1.5">
                {project.name}
              </Link>
          ...
          <motion.button
            ...
            className="p-1.5 rounded-lg text-slate-500 hover:text-rose-400 hover:bg-rose-500/10 opacity-0 group-hover:opacity-100 transition cursor-pointer"
      ...
      <div className="px-5 py-3 bg-slate-900/60 border-t border-white/[0.05] flex items-center justify-between">
        ...
        <Link
          ...
          className="text-xs font-medium text-cyan-400 group-hover:text-cyan-300 flex items-center gap-1 transition-all"
        >
